...
<#
.SYNOPSIS
    Initializes a legacy (Win7) Outlook Data Folder Container
...
function Initialize-LegacyODFC
{
    ...
    else
    {
        ...
        elseif ($PSCmdlet.ShouldProcess($MountPoint, 'DELETE CONTENTS'))
        {
            ...
            catch { throw "Error clearing ODFC mount point '$MountPoint'!`n$($_.Exception.Message)" }
        }
    }
...
<#
.SYNOPSIS
    DiskPart.exe wrapper that's slightly less bad than plain old DiskPart.
...
<#
.SYNOPSIS
Spits a summary out to the Windows event log.

.PARAMETER LogName
    Specifies the name of the log to which the event is written.

.PARAMETER Source
    Specifies the event source, which is typically the name of the application that
    is writing the event to the log.
...
.PARAMETER EntryType
    Specifies the entry type of this event.

.PARAMETER Message
    Specifies the event message.

.PARAMETER TranscriptFile
    Specifies the path to a file with supplemental details, such as a PowerShell
    transcript file. This will be appended to the specified event message.
#>
function Write-Log
{
    [CmdletBinding()]
...
<#
.SYNOPSIS
    Drops a love note alongside the VHD container whenever whenever
    it is successfully attached.

.DESCRIPTION
    Contains information about the client to help with troubleshooting
    issues such as locked VHD files.

.PARAMETER FilePath
    Specifies the path to the VHD file

.PARAMETER Extension
    Specifies the file extension for the love note

.PARAMETER Method
    Indicates the method used to create and/or attach the VHD. For now,
    the only valid option is 'DiskPart', until I have time to focus on Win10
    implementation (which will likely use the native storage cmdlets).

.PARAMETER Context
    Describes the context under which the disk was attached:
        CreateNew: VHD was created immediately before attaching
        Reattach:  An existing VHD was reattached
#>
